...
import { installImportBrowser } from '../../../../../tests/importTestSupport';
...
beforeEach(() => { installImportBrowser(); });
afterEach(() => { cleanup(); vi.restoreAllMocks(); vi.unstubAllGlobals(); vi.mocked(createInvitation).mockReset(); });
...
it('no genera claves antes de confirmar; una sesión nueva limpia resultados y usa nuevas claves', async () => {
  ...
  act(() => { expect(result.current.reset()).toBe(false); });
  await act(async () => { await result.current.remove(true); });
